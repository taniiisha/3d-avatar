angular
  ...
  .controller("InterviewController", function ($scope) {
    ...
    // Increase pixel ratio for crispness (warning: high GPU usage)
    ...
    directionalLight.shadow.mapSize.width = 1024; // High res shadow map
    ...
    function launchAvatar(url) {
      gltfLoader.load(
        ...
        (gltf) => {
          ...
          // 4. Traverse the model to enable shadows and tweak materials
          model.traverse((child) => {
            if (child.isMesh) {
              // Avatar casts and receives its own shadows
              child.castShadow = true;
              child.receiveShadow = true;
              
              // Optional material tweaking: make skin a bit tighter and less shiny if it's generic
              if (child.material) {
                if (child.name.includes("Head") || child.name.includes("Body")) {
                    child.material.roughness = 0.6; // Softer skin
